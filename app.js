...
// Set up date inputs safely inside event context or conditional checks
...
function renderFutureWidget() {
  const box = document.getElementById("motivation-card");
  if (!box) return;

  if (!appData.futureNotes || appData.futureNotes.length === 0) {
    box.textContent = "No Future Notes Yet";
    return;
  }

  const latest = appData.futureNotes[appData.futureNotes.length - 1];
  box.textContent = latest.message || latest.text || "Keep Going.";
}
...
// Expose core structural configurations to window scope
